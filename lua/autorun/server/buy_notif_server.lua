...
local buyNotificationDebug = CreateConVar("ttt_buy_notification_debug","0", {FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE, FCVAR_NOTIFY}, "Should TTT Buy Notifications DEBUG MODE be active?"):GetBool()
...
local function SendBoughtNotif(ply_o, equipment, is_item)
  ...
  for _, ply in pairs( player.GetAll() ) do
    ...
    if IsValid( ply ) and ply:IsActive() and ( ( ply.GetTeam and ply:GetTeam() == ply_o:GetTeam() and not ply_o:GetDetective() ) or ( ply:GetRole() == ply_o:GetRole() ) ) and ( buyNotificationDebug or ply != ply_o ) then
